...
class TrainingLoop:
    def __init__(
        ...
        *,
        ...
        if self._use_wandb:
            run_name = f"AES_DW{domain_weight}_DS{domain_grouping_strategy}_DN{actual_domain_nums}_S{seed}"
            wandb.init(
    ...
    def _evaluate(self, dataloaders, prefix: str):
        ...
        for dataloader in dataloaders:
            ...
            for batch in tqdm.tqdm(dataloader, ncols=0, desc=f"{prefix} on {dataloader.dataset.dataset_name}", unit='step'):
                ...
                with torch.no_grad():
                    output, _ = self._model(feature) # Ignore domain logits
