...
from dataset.dataset_AES_multi import AESFeaturesMulti
from model.model_domainGRL_multi import DANNProjectionHeadMulti
...
def infer_domain_configs_from_checkpoint(checkpoint_path):
    """
    讀取 Checkpoint 結構，自動偵測 Domain Heads 的設定。
    """
    print(f"Inspecting checkpoint: {checkpoint_path}")
    # Load on CPU to verify structure
    try:
        ckpt = torch.load(checkpoint_path, map_location='cpu')
    except Exception as e:
        print(f"Error loading checkpoint for inspection: {e}")
        return {}
    ...
    # 1. Group keys by head name
    for key in state_dict.keys():
        if key.startswith("domain_classifiers."):
            # key example: domain_classifiers.kmeans_4.0.weight
            parts = key.split('.')
            if len(parts) >= 2:
                name = parts[1] # e.g., 'kmeans_4', 'gt'
                if name not in classifiers:
                    classifiers[name] = []
                classifiers[name].append(key)
    
    configs = {}
    
    # 2. Determine output size for each head
    for name, keys in classifiers.items():
        # Find weight keys
        weight_keys = [k for k in keys if k.endswith('.weight')]
        if not weight_keys: continue
        
        # Find the last layer (largest index)
        def get_layer_idx(k):
            try: return int(k.split('.')[2])
            except: return -1
                
        last_weight_key = max(weight_keys, key=get_layer_idx)
        
        # Output size is the first dimension of the weight matrix
        weight_tensor = state_dict[last_weight_key]
        num_classes = weight_tensor.shape[0]
        
        configs[name] = num_classes
        
    print(f"-> Auto-detected domain configs: {configs}")
    return configs
...
def main():
    ...
    gin.bind_parameter('AESFeaturesMulti.dataset_name', args.dataset_name)
    gin.bind_parameter('AESFeaturesMulti.data_path', args.data_path)
    # 確保 debug 模式關閉，讀取全量資料
    try:
        gin.bind_parameter('AESFeaturesMulti.debug', False)
    except:
        pass # 如果 config 沒定義 debug 可能會報錯，忽略即可

    # 2. Initialize Dataset & Dataloader
    print(f"Initializing Dataset: {args.dataset_name}")
    dataset = AESFeaturesMulti() 
    ...
    # 4. Auto-detect Domain Configs
    domain_configs = infer_domain_configs_from_checkpoint(args.checkpoint_path)
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader):
            ...
            out_tuple = model(features, domain_weight=0.0) 
