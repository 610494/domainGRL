...
import gin
# [修改] 引用 Proto 版本的 Model
import model.model_domainGRL_proto as model_lib
import numpy as np
...
class TrainingLoop:
    def __init__(
        ...
        *,
        ...
        # [修改] 自動偵測 Domain 數量 (針對 GT 策略) - 參照你的範例
        actual_domain_nums = domain_nums # Default from args
        if hasattr(train_dataset, 'num_domains'):
            ...
            # 如果是 GT 策略，必須使用偵測到的數量
            if domain_grouping_strategy == 'GT' or actual_domain_nums is None:
                actual_domain_nums = detected_domains
                # [關鍵修正] 動態綁定參數給 PrototypicalDANNHead
                gin.bind_parameter('PrototypicalDANNHead.num_domains', actual_domain_nums)
                print(f"[Gin Override] PrototypicalDANNHead.num_domains set to {actual_domain_nums}")

        # Initialize WandB after knowing actual domain nums
        self._use_wandb = use_wandb
        if self._use_wandb:
            run_name = f"AES_Full_DW{domain_weight}_PD{proto_div_lambda}_NP{num_prototypes}_DS{domain_grouping_strategy}_DN{actual_domain_nums}_S{seed}"
            wandb.init(
        ...
        # [修改] AES Validation/Test sets - 參照你的範例
        ...
        self._proto_div_lambda = proto_div_lambda # Proto specific
    ...
    def _train_once(self, batch) -> Dict[str, float]:
        ...
        if self._loss_type == 'mgnll':
            ...
            (means, _), domain_logits = self._model(features, domain_weight=self._domain_weight)
            loss_main = self._loss_fn(means, labels) 
        
        loss_domain = self._domain_loss_fn(domain_logits, domain_labels)
        
        # [新增] Proto Diversity Loss
    ...
    def _evaluate(self, dataloaders, prefix: str):
        ...
        for dataloader in dataloaders:
            ...
            for batch in tqdm.tqdm(dataloader, ncols=0, desc=f"{prefix} on {dataloader.dataset.dataset_name}", unit='step'):
                # Handle return of 2 or 3 items
                if len(batch) == 3: feature, label, _ = batch
                else: feature, label = batch
                
                feature = feature.to(self._device)
                with torch.no_grad():
                    # Proto Model Output: ((mean, cov), domain_logits)
                    (means, _), _ = self._model(feature) 
                    
                    output = means.cpu().numpy()
            ...
            for name in label_names:
                ...
                if np.std(pred) > 1e-6 and np.std(target) > 1e-6:
                    pcc = np.corrcoef(target, pred)[0][1]
                    srcc = stats.spearmanr(target, pred)[0]
                else:
                    pcc, srcc = 0.0, 0.0
...
def main():
    ...
    parser.add_argument('--gin_path', type=str, default='configs/domainGRL_proto.gin')
    ...
    # [修改] 針對 Proto Model 的綁定
    if args.domain_nums is not None:
        gin.bind_parameter('AESFeatures.num_random_domains', args.domain_nums)
        # 這裡改用 PrototypicalDANNHead
        gin.bind_parameter('PrototypicalDANNHead.num_domains', args.domain_nums)
    ...
    if args.num_prototypes:
        ...
        gin.bind_parameter('PrototypicalDANNHead.num_prototypes', args.num_prototypes)
    ...
    train_loop = TrainingLoop(
        ...
        domain_weight=args.domain_weight if args.domain_weight else 1.0,
