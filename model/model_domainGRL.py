"""
AES DANN Model: Domain Adversarial Training (GRL) without Prototypes.
Adapts features to be domain-invariant using a Gradient Reversal Layer.
Supports AES dataset (4 dimensions, 1-10 scale).
"""
...
class DANNProjectionHead(nn.Module):
    def __init__(
        ...
        dense_neurons: Sequence[int] = (64, 32, 14), # AES Probabilistic (4 mean + 4 var + 6 cov = 14)
        ...
        # Determine targets (AES: 14 out -> 4 targets; NISQA: 20 out -> 5 targets)
        if self._output_dim == 14: self._n_targets = 4
        elif self._output_dim == 20: self._n_targets = 5
        elif self._output_dim == 4: self._n_targets = 4 # Non-prob AES
        elif self._output_dim == 5: self._n_targets = 5 # Non-prob NISQA
        else: self._n_targets = 4 # Fallback
