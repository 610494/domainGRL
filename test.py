...
def analyze_candidate(cand_path, exp_name, baseline_data):
    ...
    for asp in ASPECTS:
        ...
        # 4. Check Significance Conditions
        # (A) MSE: p < 0.05 AND Method Error < Base Error
        is_mse_better = (p_mse < ALPHA) and (mse < base_mse_val)
        
        # (B) SRCC: p < 0.05 AND Method SRCC > Base SRCC (Williams is two-sided)
        is_srcc_better = (p_srcc < ALPHA) and (srcc > base_srcc_val)
        
        # (C) PCC Significance (for 'pass_all' logic)
        is_pcc_better = (p_pcc < ALPHA) # Assuming higher is better check is implicit
        
        if not is_mse_better:
            cand_res['pass_mse'] = False
            
        if not is_srcc_better:
            cand_res['pass_srcc'] = False
        
        if not (is_mse_better and is_pcc_better and is_srcc_better):
            cand_res['pass_all'] = False

    cand_res['srcc_avg'] = np.mean(srcc_list) if srcc_list else 0.0
